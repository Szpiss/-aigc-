...
该项目定位为“基于 AIGC 与自适应调度的英语词汇学习对战小程序”。项目基于「微信小程序」原生框架与「微信小程序云开发」实现，包含：单词对战（随机/好友/人机）、词汇学习、生词本、排行榜、设置、数据看板，以及 AIGC 学习助手。系统具备完整的数据采集、统计分析与自适应学习闭环。


## 项目痛点与解决方案

面向大学生英语词汇学习动力不足、反馈滞后、AI 工具与学习数据割裂的问题，本项目设计了“学习—对战—数据采集—自适应调度—Agent 复盘”的闭环式英语词汇学习系统。

- **学习动力不足**：通过单词对战和排行榜增强即时反馈。
- **复习内容不精准**：通过 `wordMastery` 和 `learningPlan` 将错词、弱词回流到学习计划。
- **AI 建议脱离真实数据**：AIGC 助手读取学习计划、弱词榜、学习报告和对战摘要，生成个性化学习建议和对战复盘。
